...
class MatchaHybrid(nn.Module):
    ...
    def inference(
        ...
        w = self.duration_predictor(mu_x, reverse=True, noise_scale=noise_scale)
        w = w * length_scale
...
class Log(nn.Module):
    ...
    def forward(self, x, cond, g, reverse=False):
        return x, 0.0
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths):
        return torch.randn(x.shape[0], 192, x.shape[1]), None, None
...
class CFMFlowDecoder(nn.Module):
    ...
    def forward(self, mu, y, lengths):
        return torch.tensor(0.0)

    def solve_ode(self, mu, steps, temperature):
