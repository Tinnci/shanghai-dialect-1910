...
def analyze_phonetic_consistency(lessons: List[LessonFile], output_json: str = "phonetic_analysis.json"):
    """
    高级上海话音韵一致性分析
    """
    # 1. 数据收集
    ...
    for item in sorted(report['typos'], key=lambda x: -x['similarity'])[:10]:
        print(f"  - 「{item['char']}」: {item['main']} → {item['alt']} ({item['alt_count']}次) | Sim: {item['similarity']}")
        ex = item['examples'][0]
