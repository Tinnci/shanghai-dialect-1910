...
def build_pronunciation_db(lessons: List[LessonFile]) -> Dict[str, CharPronInfo]:
    ...
    char_data = defaultdict(
        lambda: {"counts": defaultdict(int), "examples": defaultdict(list)}
    )
...
def analyze_ruby_pair(
    pinyin: str, hanzi: str, pron_db: Dict[str, CharPronInfo]
    ...
    if len(h_chars) > len(p_parts):
        ...
        if not missing_chars:
            ...
            safety = (
                ...
                else SafetyLevel.REVIEW
                if min_confidence > 0.7
                else SafetyLevel.MANUAL
            )
            return FixSuggestion(
                file="",
                line_num=0,
...
def scan_file_for_fixes(
    lesson: LessonFile, pron_db: Dict[str, str]
) -> List[FixSuggestion]:
    """扫描单个文件，返回所有修复建议"""
    suggestions = []
    ...
    for filename, fixes in by_file.items():
        ...
        for i, fix in enumerate(fixes, 1):
            ...
            safety_icon = (
                ...
                if fix.safety == SafetyLevel.REVIEW
                else "🔴"
            )
            ...
            if fix.needs_input:
                if interactive:
                    user_input = input(
                        f"  输入 '{fix.missing_char}' 的拼音 (留空跳过): "
                    ).strip()
                    if user_input:
                        print(
                            f'  → 需手动编辑文件添加: #r("{user_input}", "{fix.missing_char}")'
                        )
                    skipped_count += 1
                else:
                    print("  [需手动处理]")
