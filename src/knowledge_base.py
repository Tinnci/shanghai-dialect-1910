...
class KnowledgeBase:
        ...
        if self.rules_file.exists():
            try:
                with open(self.rules_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.initial_rules = [
                        TransformRule(**r) for r in data.get("initial_rules", [])
                    ]
                    self.final_rules = [
                        TransformRule(**r) for r in data.get("final_rules", [])
                    ]
                    self.config = KnowledgeConfig(**data.get("config", {}))
                print(
                    f"📚 已加载知识库: {len(self.initial_rules)} 条声母规则, {len(self.final_rules)} 条韵母规则"
                )
    ...
    def update_rules(
        self, initial_rules: List[TransformRule], final_rules: List[TransformRule]
    ):
        """更新规则库（只保留高置信度规则）"""
        # 过滤低置信度规则
        self.initial_rules = [
            r for r in initial_rules if r.confidence >= self.config.confidence_threshold
        ]
        self.final_rules = [
            r for r in final_rules if r.confidence >= self.config.confidence_threshold
        ]
