...
try:
    from src.romanization import split_syllable_church  # noqa: F401
    from src.rule_induction import PhoneticFeatures, INITIAL_FEATURES, FINAL_FEATURES  # noqa: F401
except ImportError:
...
def test():
    converter = PottToIPA()
    test_cases = ["ngoo", "tshang", "nyih", "kuh"]
    ...
    for pott in test_cases:
        ...
        print(
            f"{pott:10} -> IPA: {ipa:10} | Modern (Wugniu): {wugniu:10} (Conf: {conf:.2f})"
        )
