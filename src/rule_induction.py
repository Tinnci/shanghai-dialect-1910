...
class PhoneticFeatures:
    ...
    def to_vector(self) -> List[float]:
        """转换为数值向量，用于计算距离"""
        place_map = {"labial": 0, "dental": 1, "velar": 2, "palatal": 3, "glottal": 4, "": -1}
        manner_map = {"stop": 0, "fricative": 1, "affricate": 2, "nasal": 3, "lateral": 4, "approximant": 5, "": -1}
...
INITIAL_FEATURES = {
    ...
    # 齿龈塞擦/擦音
    "ts": PhoneticFeatures(place="dental", manner="affricate", voiced=False, aspirated=False),
    "tsh": PhoneticFeatures(place="dental", manner="affricate", voiced=False, aspirated=True),
    "c": PhoneticFeatures(place="dental", manner="affricate", voiced=False, aspirated=False),
    "ch": PhoneticFeatures(place="dental", manner="affricate", voiced=False, aspirated=True),
    "dz": PhoneticFeatures(place="dental", manner="affricate", voiced=True, aspirated=False),
    "j": PhoneticFeatures(place="dental", manner="affricate", voiced=True, aspirated=False),
    ...
    "ky": PhoneticFeatures(place="palatal", manner="stop", voiced=False, aspirated=False),
...
FINAL_FEATURES = {
    ...
    "oq": PhoneticFeatures(vowel_height="mid", vowel_front="back", vowel_round=True, stop_coda=True),
    "uq": PhoneticFeatures(vowel_height="high", vowel_front="back", vowel_round=True, stop_coda=True),
    ...
    "oh": PhoneticFeatures(vowel_height="mid", vowel_front="back", vowel_round=True, stop_coda=True),
    "uh": PhoneticFeatures(vowel_height="high", vowel_front="back", vowel_round=True, stop_coda=True),
    ...
    "on": PhoneticFeatures(vowel_height="mid", vowel_front="back", vowel_round=True, nasal_coda=True),
...
def phonetic_feature_similarity(init1: str, final1: str, init2: str, final2: str) -> float:
    """
    基于音韵特征计算两个音节的相似度
...
class RuleInductionEngine:
    ...
    def add_parallel_pair(self, church_pinyin: str, wugniu_pinyin: str, hanzi: str = ""):
        """添加一对平行拼音用于学习"""
    ...
    def _split_syllable(self, syllable: str, is_wugniu: bool = False) -> Tuple[str, str]:
        """分离声母和韵母"""
        ...
        # 声母列表 (按长度排序)
        if is_wugniu:
            initials = ["tsh", "dz", "ts", "ng", "gn", "kh", "ph", "th", "gh", "ch", "sh", "zh",
                       "k", "h", "m", "n", "l", "v", "w", "f", "p", "b", "t", "d", "s", "z", "j", "c", "q", "x", "y", "'"]
        else:
            initials = ["tsh", "dz", "ts", "ng", "ny", "kh", "ph", "th", "gh", "ch", "sh", "ky", "hy", "kw", "gw", "hw",
                       "k", "h", "m", "n", "l", "v", "w", "f", "p", "b", "t", "d", "s", "z", "j", "y", "'"]
    ...
    def print_rules_report(self):
        ...
        for rule in initial_rules[:20]:
            conf = "🟢" if rule.confidence > 0.8 else "🟡" if rule.confidence > 0.5 else "🔴"
...
class IntelligentAnalyzer:
    ...
    def analyze_with_heuristics(self, church_pinyin: str, hanzi: str) -> Dict:
        ...
        if hanzi in self.char_pinyins:
            ...
            c_init, c_final = self.rule_engine._split_syllable(church_pinyin, is_wugniu=False)
            
            for variant in wugniu_variants:
                w_init, w_final = self.rule_engine._split_syllable(variant, is_wugniu=True)
            ...
            if best_sim > 0.7:
                result["reasons"].append(f"音韵特征匹配: '{church_pinyin}' ≈ '{best_match}' (相似度: {best_sim:.2f})")
            else:
                result["is_valid"] = False
                result["confidence"] *= best_sim
                result["reasons"].append(f"最接近的读音是 '{best_match}' (相似度: {best_sim:.2f})")
                result["suggestions"].append(f"建议检查: {wugniu_variants}")
        
        # 启发式规则3: 入声韵尾检查
        if church_pinyin.endswith(('h', 'k')) and not church_pinyin.endswith(('ng', 'ung', 'aung')):
