        ...
        for img_idx, img_info in enumerate(image_list):
            ...
            try:
                ...
                if base_image:
                    ...
                    # Save image
                    image_filename = (
                        f"page_{page_num + 1:04d}_img_{img_idx + 1:03d}.{image_ext}"
                    )
                ...
                print(
                    f"    ⚠️ Error extracting image {img_idx + 1} from page {page_num + 1}: {e}"
                )
...
def categorize_pages_by_content(pdf_doc, output_dir: Path, log: list):
    ...
    if toc:
        ...
        for item in toc:
            ...
            toc_log.append({"level": level, "title": title, "page": page})
            print(f"    {'  ' * (level - 1)}[{page}] {title}")
        log.append({"type": "table_of_contents", "entries": toc_log})
    else:
...
def run_extraction(project_root: Path):
    ...
    render_dpi = 150
    pages_rendered = render_full_pages(
        pdf_doc, output_base, extraction_log, dpi=render_dpi
    )
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        json.dump(
            {"metadata": stats, "pdf_info": pdf_info, "log": extraction_log},
            f,
            indent=2,
            ensure_ascii=False,
        )
